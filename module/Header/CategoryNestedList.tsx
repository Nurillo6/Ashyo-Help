        ...
        <div className={`containers !px-0 w-full !z-[999999] mx-auto right-0 left-0 duration-400 absolute top-0 sm:top-[146px] ${showCategory ? "h-[100vh] sm:h-[570px]" : "h-0 opacity-0"} overflow-hidden flex  bg-white shadow-md`}>
            <ul className='w-[52%] space-y-[15px] sm:space-y-0 sm:w-[35%] py-[13px] sm:py-[43px] px-[5px] sm:px-[32px] bg-[#ebeff3]'>
                {categories.length > 0 ? categories.map((item:CategoryType, index:number) => (
                    <li onClick={() => handleCategoryClick(item.id)} onMouseEnter={() => showCategoryChildren(item)} key={index} className='flex cursor-pointer items-center text-[14px] hover:bg-white rounded-md duration-300 sm:text-[16px] gap-[15px] py-[8px] sm:py-[12px] pl-[10px] sm:pl-[40px]'>
            ...
            <ul className='w-[48%] space-y-2 sm:space-y-[5px] sm:w-[65%] py-[13px] sm:py-[55px] px-[22px] sm:px-[73px] bg-white'>
                ...
                {nestedChild?.children?.map((item:CategoryType) => (
                    <li onClick={() => handleCategoryClick(item.id)} key={item.id} className='cursor-pointer py-[10px] pl-[5px] duration-300 rounded-md hover:bg-[#EBEFF3] text-[14px] sm:text-[16px]'>{item?.name}</li>
                ))}
            </ul>
        </div>
    </div>
